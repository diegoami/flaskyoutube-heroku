...
app.secret_key = 'E8OusFzWtzQTE72RvcHv'
...
def execute_command(http_auth ):
  youtube = Youtube(discovery.build(YOUTUBE_API_SERVICE_NAME , YOUTUBE_API_VERSION, http=http_auth))
...
@app.route('/reset')
def reset():
    if 'credential' in flask.session:
